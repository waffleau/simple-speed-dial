...
function addNewEntryButton(entryArray) {
  var newEntry = $('<div class="entry" id="new_entry" title="Add New"><div><span class="fa fa-plus"></span></div></div>');
...
function addSpeedDialBookmark(bookmark, entryArray) {
  ...
  var bookmarkUrl = 'custom_icon_data_' + bookmark.url;
...
function createSpeedDial(folderId) {
  ...
  chrome.bookmarks.getSubTree(folderId, function (node) {
    ...
    if (localStorage.getItem('show_options_gear') === 'true' && $('#options').children().length === 0) {
      $('#options').append($('<a class="fa fa-gear" href="/views/options.html" title="Options"></a>'));
    }
...
function updateCustomIcon(url, oldUrl) {
  var iconObject = JSON.parse(localStorage.getItem('custom_icon_data'));
  var iconUrl = $('#bookmark_form .icon').prop('value').trim();
